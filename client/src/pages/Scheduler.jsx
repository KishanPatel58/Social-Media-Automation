...
const Scheduler = () => {
  ...
  const togglePlatform = (id) => setSelectedPlatforms((prev) => (prev.includes(id) ? prev.filter((platform) => platform !== id) : [...prev, id]))
    ...
    <div className="flex flex-col lg:flex-row gap-6 h-full">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 !p-6">
          ...
          <form onSubmit={handleSchedule} className="space-y-5">
            ...
            <div>
              ...
              <textarea name="" className="w-full !px-5 !py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 text-sm placeholder-slate-400 outline-none resize-none" required rows={5} placeholder="what do you want to share today ?" value={content} onChange={(e) => setContent(e.target.value)} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="date" required className="w-full !pl-10 !pr-4 !py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-none" onChange={(e) => setScheduledDate(e.target.value)} value={scheduledDate} />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="time" required className="w-full !pl-10 !pr-4 !py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-none" onChange={(e) => setScheduledTime(e.target.value)} value={scheduledTime} />
                </div>
              </div>
            </div>
      ...
      <div className="flex-1 flex flex-col gap-6 min-w-0">
          ...
          <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
            {scheduled.length === 0 ? (
              ...
              scheduled.map((post,idx)=> (
                <div key={idx} className="!px-5 !py-4 hover:bg-slate-50/60 transition-colors">
                  <div className="flex items-center justify-between !mb-2">
                    <div className="flex gap-1.5 items-center">
                      {
                        post.platform?.map((pl)=>{
                          const meta = PLATFORMS.find((platform)=>platform.id === pl)
                          return meta ? <meta.icon key={idx} className="size-3.5 text-slate-400"/> : null
                        })
                      }
                    </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="max-h-72 overflow-y-scroll divide-y divide-slate-50">
            {published.length === 0 ? (
