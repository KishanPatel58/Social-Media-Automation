...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const { logout, user } = {
    ...
    user: { name: "Kishan Patel", email: "patelkishan@gmail.com" }
  }
  const location = useLocation();
  const navlinks = [
    { name: "Dashboard", to: "/dashboard", icon: LayoutDashboardIcon },
    { name: "Accounts", to: "/accounts", icon: UsersIcon },
    { name: "Scheduler", to: "/schedule", icon: CalendarDaysIcon },
    { name: "AI Composer", to: "/ai-composer", icon: Wand2Icon }
  ]
  return (
    <div className={`fixed !inset-y-0 left-0 !z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-full transform transition-transform duration-200 ease-in-out md:relative md:!translate-x-0 ${isOpen ? "!translate-x-0" : "!-translate-x-full"}`}>
      ...
      <nav className="flex-1 !px-3 !space-y-1">
        {
          navlinks.map((link, idx) => {
            ...
            return (
              <NavLink key={idx} to={link.to} end={link.to === "/dashboard"} onClick={() => setIsOpen(false)} className={`flex items-center gap-3 !px-3 !py-2.5 rounded text-sm transition-all duration-150 border ${isActive ? "bg-red-50 text-red-600 border-red-100" : "text-slate-500 hover:bg-slate-50 border-transparent hover:text-slate-700"}`}>
                <link.icon className={`size-4.5 shrink-0 ${isActive ? "text-red-500" : "text-slate-500"}`} />
                {link.name}
      ...
      <div className="!p-4 border-t border-slate-100">
        <div className="flex items-center gap-3 !p-2 rounded-xl hover:bg-slate-50 transition-color">
          <div className="size-8 rounded-full bg-linear-to-br from-red-400 to bg-pink-400 flex items-center justify-center text-white text-sm font-medium shrink-0">
            {
