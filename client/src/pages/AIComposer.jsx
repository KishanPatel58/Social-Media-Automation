    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        ...
        <div className="relative group !mt-12">
          <textarea name="" rows={10} placeholder="Share your idea.. (e.g. A post about the launch of our new eco-friendly coffee beans)" value={prompt} onChange={(e) => setPrompt(e.target.value)} className="w-full !p-6 bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 outline-none focus:border-slate-400 transition resize-none h-40" />
          <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
            ...
            <button className="bg-slate-900 hover:bg-slate-800 text-white flex items-center gap-2 !px-4 !py-2 rounded-lg" disabled={loading} onClick={() => handleGenerate()}>
              {loading ? (
                <>
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {tones.map((tones, idx) => (
            <button key={idx} onClick={() => setTone(tones)} className={`!px-4 !py-1.5 rounded-full text-sm transition-all border ${tone === tones ? "bg-red-500 border-red-500 text-white" : "bg-white border-slate-200 text-slate-500 hover:border-slate-300"}`}>{tones}</button>
          ))}
        </div>
      </div>
      {/* AI Generated Posts */}
      <div className="space-y-6 !pt-12 border-t border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {generations.map((generation, idx) => (
            <div key={idx} className="group bg-white rounded-2xl border border-slate-100 !p-5 hover:border-red-200 transition-all relative">
              <div className="flex flex-col h-full space-y-4">
                ...
                {generation.mediaUrl && (
                  <div className="rounded-xl border border-slate-50 bg-slate-50">
        ...
        activeScheduler && (
          <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center !p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="!p-8 bg-slate-50/50 bordere-t border-slate-50 space-y-8">
                {/* Options */}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {
                        PLATFORMS.map((platform,idx)=>{
                          ...
                          return (
                            <button key={idx} className={`!p-2.5 rounded-md border text-xs ${active ? "bg-red-500/80 text-white" : "bg-white border-slate-200 text-slate-400 hover:border-slate-300"}`} onClick={()=>setSelectedPlatforms((prev)=>(prev.includes(platform.id) ? prev.filter((x)=>x!==platform.id) : [...prev,platform.id]))}>
