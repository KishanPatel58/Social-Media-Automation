      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-100">
        ...
        <div className="!p-6 flex flex-col gap-2">
          {PLATFORMS.map((platform, idx) => {
            ...
            return (
              <button key={idx} className={`flex items-center gap-3 !p-3.5 rounded-xl border text-left transition-all ${isConnected ? "border-red-200 bg-red-50 cursor-default" : "border-slate-200 bg-slate-50 hover:border-slate-300 hover:bg-slate-100 cursor-pointer"} ${isConnecting && "opacity-60"}`} disabled={isConnected || isConnecting} onClick={() => onConnect(platform.id)}>
                {/* Icon */}
                <div className="!p-2">
                  <platform.icon className={`size-5 ${isConnected ? "text-red-600" : "text-slate-500"}`} />
                </div>
                {/* Label */}
